...
from crime.crime.constants import all_raw_pkl_file_name, subset_raw_pkl_file_name, raw_url, crime_folder_name, \
    html_file_name, MONTH, LONGITUDE, LATITUDE, CRIME_TYPE, FALLS_WITHIN, COLOR
...
def get_date_from_string(df: pd.DataFrame):
    df[MONTH] = pd.to_datetime(df[MONTH], format='%Y-%m')


def create_colors_for_items(items: list) -> dict:
...
def build_string_for_location(row: pd.Series) -> str:
    return f'{row.Latitude},{row.Longitude}'


def build_string_for_marker(row: pd.Series) -> str:
    return f'markers=color:{row.Color}%7Csize:tiny'


def apply_filters(df: pd.DataFrame, filter_falls_within: str, filter_month: datetime.date, centre: Location,
...
def build_df_marker(df: pd.DataFrame) -> pd.DataFrame:
...
def build_string_for_url(df: pd.DataFrame, crime_type: str) -> str:
...
def build_url_parameters(centre: Location, view: View, api_key: str) -> tuple:
...
def build_url_for_crime_type(df: pd.DataFrame, crime_type: str, raw_url: str, map_params: str, key_params: str) -> str:
    # build string for location
...
def build_html_for_crime_type(url: str, crime_type: str, filter_month: datetime.date, html_template_body: str):
...
def build_html_file(df: pd.DataFrame, df_marker: pd.DataFrame, filter_month: datetime.date,
                    html_template_body: str, raw_url: str, centre: Location, view: View, api_key: str) -> str:
...
def write_to_file(html_file: str, pathfile: str):
